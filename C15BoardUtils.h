...
Board * createBoard(int rows, int cols){
	...
	changeThis->emptyTileCol = changeThis->emptyTileRow = 0;
	changeThis->table = (int**)malloc(rows*sizeof(int*));
	for (int i = 0; i < rows; i++)
		changeThis->table[i] = (int*)malloc(cols* sizeof(int));
...
void initializeBoard(Board * source, int *tobeCopiedArray, int rows, int cols){
	if (source == NULL || tobeCopiedArray == NULL || rows <= 1 || cols <= 1)
	{
		return;
	}
	int count = 0;
	for (int i = 0; i < rows; i++)
	{
		...
		{
			source->table[i][j] = tobeCopiedArray[count++];
...
int checkBoard(Board * board){
	...
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
		{
			if (counter != board->table[i][j] && board->table[i][j] != 0)
				return 0;
			counter++;
		}
	}
